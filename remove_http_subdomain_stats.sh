...
[ -z "`query "select domain from http_subdomains where domain='$opt_domain_val' and subdomain='$opt_subdomain_val';"`" ] && echo "ERROR : Service http for subdomain $opt_subdomain_val of $opt_domain_val is disabled" && exit 1
...
[ -z "`query "select domain from http_subdomains_stats where domain='$opt_domain_val' and subdomain='$opt_subdomain_val' and  engine='$opt_engine_val';"`" ] && echo "ERROR : Service Stats with engine $opt_engine_val for  subdomain $opt_subdomain_val of domain $opt_domain_val is not enabled" && exit 1
...
query "delete from http_subdomains_stats where engine='$opt_engine_val' and domain='$opt_domain_val' and subdomain='$opt_subdomain_val';" || exit 1
