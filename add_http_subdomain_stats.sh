...
[ -z "`query "select domain from http_subdomains where domain='$opt_domain_val' and subdomain='$opt_subdomain_val';"`" ] && echo "ERROR : Service HTTP for subdomain $opt_subdomain_val of domain $opt_domail_val is disabled" && exit 1
[ -z "`query "select name from stat_engines where name='$opt_engine_val';"`" ] && echo "ERROR : Stat engine $opt_engine_val is unknown" && exit 1
[ -n "`query "select domain from http_subdomains_stats where domain='$opt_domain_val' and subdomain='$opt_subdomain_val' and  engine='$opt_engine_val';"`" ] && echo "ERROR : Service Stats with engine $opt_engine_val for subdomain $opt_subdomain_val of domain $opt_domain_val is already enabled" && exit 1
...
query "insert into http_subdomains_stats (domain,subdomain,engine,documentroot) values ('$opt_domain_val','$opt_subdomain_val','$opt_engine_val','$DOMAIN_POOL_ROOT/$opt_domain_val/$opt_subdomain_val/$opt_root_val');" || exit 1
