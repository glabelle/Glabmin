...
DESCRIPTION="supprimer les statistiques d'un service https"
...
[ -z "`query "select domain from https_domains_stats where domain='$opt_domain_val' and  engine='$opt_engine_val';"`" ] && echo "ERROR : Service Stats with engine $opt_engine_val for domain $opt_domain_val is not enabled" && exit 1
...
webalizer )
	rm /etc/webalizer/$opt_domain_val.https.conf
	;;
* )	;;
esac
