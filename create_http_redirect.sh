...
DESCRIPTION="créer une redirection http(s) vers un domaine ou un sous-domaine http"
USAGE="(-r|--redirection) link_name (-d|--domain) target_domain [options]"
OPTIONS="(-s|--subdomain) target_subdomain // lien vers un sous domaine http
...
[ -z "$opt_domain" ] && error "Target http domain is missing"
[ -z "$opt_redirect" ] && error "Redirection link name is missing"

#argument vs system ckeckings :
[ -z "$opt_subdomain" ] && [ -z "`query "select domain from http_domains where domain='$opt_domain_val';"`" ] && error "Service http for domain $opt_domain_val disabled"
[ -n "$opt_subdomain" ] && [ -z "`query "select domain from http_subdomains where domain='$opt_domain_val' and subdomain='$opt_subdomain_val';"`" ] && error "Service http for subdomain $opt_subdomain_val of $opt_domain_val disabled"
