...
DESCRIPTION="desactiver un sous-domaine"
...
[ -n "`query "select name from subdomains where name='$opt_subdomain_val' and domain='$opt_domain_val' and status='disabled';"`" ] && error "Subdomain $opt_subdomail_val is already disabled"
...
query "update subdomains set status='disabled' where name='$opt_subdomain_val' and domain='$opt_domain_val';"
