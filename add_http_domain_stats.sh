...
[ -n "`query "select domain from http_domains_stats where domain='$opt_domain_val' and  engine='$opt_engine_val';"`" ] && echo "ERROR : Service Stats with engine $opt_engine_val for domain $opt_domain_val is already enabled" && exit 1
